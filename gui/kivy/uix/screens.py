...
from uwallet.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds
from uwallet import bitcoin
from uwallet.util import timestamp_to_datetime
...
class SendScreen(CScreen):
    ...
    def set_URI(self, text):
        import uwallet
        try:
            uri = uwallet.util.parse_URI(text, self.app.on_pr)
        except:
    ...
    def do_save(self):
        ...
        from uwallet.paymentrequest import make_unsigned_request, PaymentRequest
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Ulord address or a payment request'))
                return
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid Ulord Address') + ':\n' + address)
                return
            try:
        ...
        if self.app.uwallet_config.get('use_rbf'):
    ...
    def _do_send(self, amount, message, outputs, rbf):
        ...
        config = self.app.uwallet_config
        try:
...
class ReceiveScreen(CScreen):
    ...
    def on_address(self, addr):
        req = self.app.wallet.get_payment_request(addr, self.app.uwallet_config)
        self.screen.status = ''
        if req:
    ...
    def get_URI(self):
        from uwallet.util import create_URI
    ...
    def save_request(self):
        ...
        self.app.wallet.add_payment_request(req, self.app.uwallet_config)
...
class RequestsScreen(CScreen):
    ...
    def update(self):
        ...
        _list = self.app.wallet.get_sorted_requests(self.app.uwallet_config) if self.app.wallet else []
        for req in _list:
    ...
    def do_view(self, obj):
        req = self.app.wallet.get_payment_request(obj.address, self.app.uwallet_config)
        status = req.get('status')
        amount = req.get('amount')
        address = req['address']
        if amount:
    ...
    def do_delete(self, obj):
        ...
        def cb(result):
            if result:
                self.app.wallet.remove_payment_request(obj.address, self.app.uwallet_config)
                self.update()
