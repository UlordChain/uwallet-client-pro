...
class BitcoinMessage(ImmutableSerializable):
    ...
    def __init__(self, message="", magic="Ulord Signed Message:\n"):
        object.__setattr__(self, 'message', message.encode("utf-8"))
        object.__setattr__(self, 'magic', magic.encode("utf-8"))

    @classmethod
    def stream_deserialize(cls, f):
        magic = BytesSerializer.stream_deserialize(f)
        message = BytesSerializer.stream_deserialize(f)
        return cls(message, magic)

    def stream_serialize(self, f):
        BytesSerializer.stream_serialize(self.magic, f)
        BytesSerializer.stream_serialize(self.message, f)
